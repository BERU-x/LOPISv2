...
function loadDashboardData(isManual = false) {
    ...
    // 2. Always set status to "Syncing..." (Yellow Dot)
    ...
    $.ajax({
        ...
        complete: function() {
            // Stop spinner after 500ms for smoothness (only if it was spinning)
...
$(document).ready(function() {
    ...
    // Expose to Master Refresher (Silent Mode)
    window.refreshPageContent = function(isManual) {
        loadDashboardData(isManual);
    };

    // Manual Click Listener (Visual Mode)
