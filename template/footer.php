...
<script src="<?php echo $web_root; ?>/assets/js/jquery-3.7.1.min.js"></script> 
    ...
    <script>
        ...
        // Ensure global access to locking variable
        window.isProcessing = false;

        // ==========================================================================
        // 1. GLOBAL SYNC UTILITY
        // ==========================================================================
        window.AppUtility = {
        ...
        const sessionWatcherInterval = setInterval(checkSessionHealth, 10000);

        // ==========================================================================
        // 3. UNIVERSAL REFRESHER (Aware of Locks)
        // ==========================================================================
        function runGlobalSync(isManual = false) {
            // ⭐ MUTEX LOCK CHECK
            // If a page-level script is currently processing (e.g. attendance_live.js), 
            // we exit immediately to prevent double-firing and "Cancelled" requests.
            if (window.isProcessing === true) {
                console.log("Global Sync: Page lock active. Skipping sync pulse.");
                return;
            }
            ...
            // Trigger Page-specific logic (like the attendance card updates)
            if (typeof window.refreshPageContent === "function") {
                window.refreshPageContent(isManual);
            }
            
            // Sync DataTables (excluding main monitor tables handled by refreshPageContent)
            if ($.fn.DataTable) {
                $('.dataTable').each(function() {
                    var dt = $(this).DataTable();
                    if (dt.ajax && dt.ajax.url() && this.id !== 'todayTable') {
                    ...
                    if (list.length && response.notifications) {
                        ...
                        if (response.notifications.length > 0) {
                            response.notifications.forEach(notif => {
                                ...
                                else if (type.includes('warning')) { bgClass = 'bg-danger'; iconClass = 'fa-exclamation-triangle'; }

                                html += `
                                    <a class="dropdown-item d-flex align-items-center" href="#" onclick="markAsRead(${notif.id}, '${notif.link}'); return false;">
                                        <div class="me-3">
                                            <div class="icon-circle ${bgClass}"><i class="fas ${iconClass} text-white"></i></div>
                                        </div>
                                        <div>
        ...
        $(document).ready(function(){
            ...
            // 🕒 INITIAL SYNC DELAY
            // Small delay allows page-level scripts to initialize and set their locks first
            setTimeout(() => { runGlobalSync(false); }, 200);
